...
- **多模式 Sheet 处理**：直接拆分、关联键匹配、基准表引用、完整保留
- **公式版 + 结果值版**：同时输出含原始公式的表格和纯值表格，支持复选
- **加密文件支持**：自动检测加密工作簿，提示输入密码后解密加载
- **输出加密**：可为输出的拆分文件设置密码保护
- **Workbook 缓存**：同一会话内避免重复加载，优化大文件预览性能
- **后台执行**：拆分任务在后台线程执行，前端实时轮询进度
- **默认输出路径**：与源文件同目录，自动创建 `{文件名}_拆分结果` 子文件夹
- **纯本地运行**：不向互联网上传任何数据
